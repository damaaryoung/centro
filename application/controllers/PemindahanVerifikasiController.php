...
class PemindahanVerifikasiController extends CI_Controller {
	...
	public function getDataAwal(){
		...
		$listJaminan = $this->PemindahanVerifikasiModel->listJaminan($kode_kantor);
		if(count($listJaminan) == 0){
			$data [] = '';
		}else{
			foreach ($listJaminan as $row) :
				$data[]    = 	['<tr> <td>'. $row['nomor']. '</td> <td>'
											...
											. '<div class="form-inline">'
												. '<form method="post" action="'. base_url().'PemindahanVerifikasiController/verifikasiMenu">'
