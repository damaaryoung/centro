...
class Update_Kirim_Jaminan_Controller extends CI_Controller {
	public function __construct() {
        parent:: __construct();
        $this->load->model('CustodianModel/update_kirim_jaminan_model');
	...
	public function index()
	{
		...
		if($session != ''){ 
            $data['getCentro'] = $this->update_kirim_jaminan_model->getCentro();
            $sysdate = $this->update_kirim_jaminan_model->sysdate();
			foreach ($sysdate as $row) :
				$data['sysdate'] = $row['sysdate'];		
			endforeach;
			$this->load->view('ViewCustodian/update_kirim_jaminan_centro.php', $data);
		}
		else{
	...
	public function getMasterJaminan(){
		...
		$getJaminanDokumen = $this->update_kirim_jaminan_model->getJaminanDokumen($kode_kantor);
	...
	public function getMasterJaminanSearch(){
		...
		$getSearchJaminanDokumen = $this->update_kirim_jaminan_model->getSearchJaminanDokumen($kode_kantor,$search);
