...
class Proses_controller extends CI_Controller
{
    ...
    function get_filter_data_proses()
    {
        $kode_kantor 	= $this->input->post('kode_kantor');
        $list = $this->Model_proses->get_filter_datatables($kode_kantor);
        $data = array();
        $no = $_POST['start'];
        foreach ($list as $field) {
            $no++;
        ...
        $output = array(
            "draw" => $_POST['draw'],
            "recordsTotal" => $this->Model_proses->count_filter_all($kode_kantor),
            "recordsFiltered" => $this->Model_proses->count_filter_filtered($kode_kantor),
            "data" => $data,
        );
        //output dalam format JSON
        echo json_encode($output);
