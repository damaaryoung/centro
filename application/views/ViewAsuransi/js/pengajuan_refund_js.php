...
var menu_kode                   = <?php echo $this->session->userdata('pengajuan_refund');?>;
...
var modal_jenis_refund_jaminan  = '';
var data                        = '';
var uploads                     = '';
var data_asuransi               = '';
var upload_update               = '';
...
$('#table_body_pengajuan_refund').on('click','.btn_delete_jaminan', function () {    
    ...
    if(status_jaminan == 'PROSES'){
        return Swal.fire({
            ...
            text: 'Status Data Sudah dalam Proses Refund Asuransi!'
        });
    }
    delete_refund(rekening,jenis,no_transaksi);
    
   
});
$('#table_body_pengajuan_refund').on('click','.btn_update_jaminan', function () {    
    rekening      = $(this).data("rekening");
    jenis         = $(this).data("jenis");
    no_reff_asuransi = $(this).data("noreff-asuransi");
    no_transaksi = $(this).data("no-trans");
...
$('#table_body_pengajuan_refund').on('click','.btn_update_jiwa', function () {    
    rekening         = $(this).data("rekening");
    jenis            = $(this).data("jenis");
    no_reff_asuransi = $(this).data("noreff-asuransi");
    no_transaksi     = $(this).data("no-trans");
...
$('#btn_simpan_modal_jaminan').click(function () {
    pengajuan_refund_jaminan_process();
});
...
function get_kode_kantor(){
    $.ajax({
            url : base_url + "Asuransi/Pengajuan_refund_asuransi_controller/get_kode_kantor",
            type : "POST",
            dataType : "json",
            timeout : 180000,
            headers: {
...
function mapping_get_data(response){
    if(menu_kode == '1'){
        for(i = 0; i < response.refund.length; i++ ){
            data += `<tr style="text-align: center;">
                    <td>${response.refund[i]['TGL_REALISASI']}</td>
                    <td>${response.refund[i]['no_rekening']}</td>
                    <td>${response.refund[i]['NAMA_NASABAH']}</td>
                    <td>${response.refund[i]['jenis_jaminan']}</td>
                    <td>${response.refund[i]['jenis_refund']}</td>
                    <td>${response.refund[i]['status_refund']}</td>
                    <td>        
                        <button type="button" class="btn btn-primary btn-sm btn_update_jaminan" id="btn_update_jaminan"
                                data-rekening="${response.refund[i]['no_rekening']}"
                                data-jenis="${response.refund[i]['jenis_asuransi']}"  
                                data-noreff-asuransi="${response.refund[i]['no_reff_asuransi']}"  
                                data-status="${response.refund[i]['status_refund']}" 
                                data-no-trans="${response.refund[i]['no_transaksi']}" 
    ...
    $('#tbl_pengajuan_refund > tbody:first').html(data);
                    
    $(document).ready(function() {
        $('#tbl_pengajuan_refund').DataTable( {
            "destroy": true,
            "scrollX": true,
            "autoWidth" : false,
            "searching": false,
...
function get_search(){
    ...
    $('#tbl_pengajuan_refund').DataTable().clear();
    $('#tbl_pengajuan_refund').DataTable().destroy();
    src_search = $('#src_search').val();
    console.log(src_kode_kantor);

    $.ajax({
            url : base_url + "Asuransi/Pengajuan_refund_asuransi_controller/get_data_search",
            type : "POST",
            dataType : "json",
            timeout : 180000,
            headers: {
...
function delete_refund(rekening,jenis,no_transaksi){
    Swal.fire({
        ...
        preConfirm: function() {
            return new Promise(function(resolve) {
                ...
                $.ajax({
                      url: base_url + "Asuransi/Pengajuan_refund_asuransi_controller/delete_pengajuan_refund",
                      type : "POST",
                      dataType : "json",
                      timeout : 180000,
                      headers: {
...
function get_data_update(rekening,jenis,no_reff_asuransi,no_transaksi){
    ...
    $('#loading-3').show();
    $.ajax({
            url : base_url + "Asuransi/Pengajuan_refund_asuransi_controller/get_data_update",
            type : "POST",
            dataType : "json",
            timeout : 180000,
            headers: {
...
function get_data_jaminan(){
    ...
    $.ajax({
            url : base_url + "Asuransi/Pengajuan_refund_asuransi_controller/get_data",
            type : "POST",
            dataType : "json",
            timeout : 180000,
            headers: {
...
function get_data_cover_jaminan(){
    ...
    $('#loading-1').show();
    $.ajax({
            url : base_url + "Asuransi/Pengajuan_refund_asuransi_controller/get_data_cover_jaminan",
            type : "POST",
            dataType : "json",
            timeout : 180000,
            headers: {
...
function pengajuan_refund_jaminan_process(){
    modal_rek_jaminan           = $('#modal_rek_jaminan').val();
    modal_reff_asuransi_jaminan = $('#modal_reff_asuransi_jaminan').val();
    modal_jenis_refund_jaminan  = $('#modal_jenis_refund_jaminan').val();
    
    
    let fd = new FormData();
    fd.append('modal_rek_jaminan',modal_rek_jaminan);
    fd.append('modal_reff_asuransi_jaminan',modal_reff_asuransi_jaminan);
    fd.append('modal_jenis_refund_jaminan',modal_jenis_refund_jaminan);
    
    $('#loading-1').show();
    $.ajax({
        url: base_url + "Asuransi/Pengajuan_refund_asuransi_controller/pengajuan_refund_jaminan_process",
        type:"POST",
        timeout : 240000,
        data:fd,
        processData:false,
...
function proses_update(){
    ...
    var jenis_refund = $('#modal_jenis_refund_jaminan_update').val();
    ...
    fd_up.append('jenis_refund',jenis_refund);
         
    $('#loading-3').show();
    $.ajax({
        url: base_url + "Asuransi/Pengajuan_refund_asuransi_controller/update_process",
        type:"POST",
        timeout : 240000,
        data:fd_up,
        processData:false,
...
function mapping_modal_jaminan_update(response){
    ...
    $('#modal_pengajuan_refund_jaminan_update').modal('show');
              
    // $('#modal_jenis_refund_jaminan_update').find('option').remove().end();
    ...
    $('#modal_jenis_refund_jaminan_update').val(response.data_details[0]['jenis_refund']);             
    // $('#modal_jenis_refund_jaminan_update').append('<option value="'+response.data_details[0]['jenis_klaim']+'" selected>' + response.data_details[0]['jenis_klaim'] +'</option>');
    // $('#modal_jenis_refund_jaminan_update').append('<option value="Kebakaran">Kebakaran</option>' +
...
function close_jaminan_update(){
    ...
    $('#modal_pengajuan_refund_jaminan_update').modal('hide');
}
function clear_modal_jaminan_update(){
    ...
    $('#modal_jenis_refund_jaminan_update').val('');
...
function get_data_cover_jiwa(){
    ...
    $('#loading-2').show();
    $.ajax({
            url : base_url + "Asuransi/Pengajuan_refund_asuransi_controller/get_data_cover_jiwa",
            type : "POST",
            dataType : "json",
            timeout : 180000,
            headers: {
...
function pengajuan_refund_jiwa_process(){
                
    modal_rek_jiwa           = $('#modal_rek_jiwa').val();
    modal_reff_asuransi_jiwa = $('#modal_reff_asuransi_jiwa').val();
    modal_jenis_refund_jiwa  = $('#modal_jenis_refund_jiwa').val();
                
    let fd = new FormData();
    fd.append('modal_rek_jiwa',modal_rek_jiwa);
    fd.append('modal_reff_asuransi_jiwa',modal_reff_asuransi_jiwa);
    fd.append('modal_jenis_refund_jiwa',modal_jenis_refund_jiwa);
      
    $('#loading-2').show();
    $.ajax({
        url: base_url + "Asuransi/Pengajuan_refund_asuransi_controller/pengajuan_refund_jiwa_process",
        type:"POST",
        timeout : 240000,
        data:fd,
        processData:false,
        ...
        error : function(response) {
            ...
            return Swal.fire({
                        icon: 'error',
                        title: 'Gagal Pengajuan Refund Asuransi Jiwa!',
                        text: 'Mohon Periksa Jaringan Anda'
            });
        }
    });
...
function proses_update_jiwa(upload_update){
    ...
    var jenis_refund = $('#modal_jenis_refund_jiwa_update').val();
...
function mapping_modal_jiwa_update(response){
    $('#modal_pengajuan_refund_jiwa_update').modal('show');
    $('#loading-4').hide();
    ...
    $('#modal_jenis_refund_jiwa_update').val(response.data_details[0]['jenis_refund']);         
    // $('#modal_jenis_refund_jiwa_update').find('option').remove().end();
    // $('#modal_jenis_refund_jiwa_update').append('<option value="'+response.data_details[0]['jenis_klaim']+'" selected>' + response.data_details[0]['jenis_klaim'] +'</option>');
    // $('#modal_jenis_refund_jiwa_update').append('<option value="Kebakaran">Kebakaran</option>' +
    //                                                   '<option value="Meninggal Dunia">Meninggal Dunia</option>' +
    //                                                   '<option value="Ketidakmampuan Tetap">Ketidakmampuan Tetap</option>' +
    //                                                   '<option value="Kehilangan">Kehilangan</option>');
...
function close_jiwa_update(){
    ...
    $('#modal_pengajuan_refund_jiwa_update').modal('hide');
}
function clear_modal_jiwa_update(){
    ...
    $('#modal_jenis_refund_jiwa_update').val('');
...
function prosees_upload_update(){
    ...
    $.ajax({
        url: base_url + "Asuransi/Pengajuan_refund_asuransi_controller/proses_upload",
        type:"POST",
        timeout : 240000,
        data:fd_up,
        processData:false,
...
function proses_delete_upload_update(){
    ...
    var proses_flag = 2;
    $.ajax({
            url : base_url + "Asuransi/Pengajuan_refund_asuransi_controller/proses_delete_upload",
            type : "POST",
            dataType : "json",
            timeout : 180000,
            headers: {
