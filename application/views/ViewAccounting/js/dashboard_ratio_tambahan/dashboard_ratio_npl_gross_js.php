                ...
                $('#btn_export_chart').click(function () 
                {
                     domtoimage.toPng(document.getElementById('reportPage'))
                        .then(function (canvas) 
                        {
                            ...
                            pdf.save("dashboard-ratio-tambahan-npl-gross.pdf");
...
//    console.log(src_kode_kantor);
   $.ajax({
           url : base_url + "Accounting/Dashboard_ratio_controller/get_data_chart_npl_gross",
           type : "POST",
           dataType : "json",
           timeout : 180000,
           headers: {
           ...
           success : function(response) {
            ...
            //    console.log("response chart Asert Total",response)
                if(!response.rasio_tambahan_npl_gross)
                {
            ...
            blue: 'rgb(54, 162, 235)'
                ...
                for (var i =0;i < response.rasio_tambahan_npl_gross.length;i++) 
                    {
                        label.push(response.rasio_tambahan_npl_gross[i]['tgl_laporan']);
                        rasio.push(response.rasio_tambahan_npl_gross[i]['rasio']);
            ...
            //Script NPAT monthly
            var data_chart_rasio_tambahan_npl_gross = 
            {
                ...
                var config_chart_rasio_tambahan_npl_gross = 
            {
                type: 'bar',
                data:data_chart_rasio_tambahan_npl_gross,
                options: {}
            };
                 ...
                 var grapharea = document.getElementById("chart_rasio_capital").getContext("2d")
                   var npat_monthly = new Chart(
                    grapharea,config_chart_rasio_tambahan_npl_gross
                    ); 
